...
import { Flag, MoreHorizontal, ShieldBan, Trash2, X } from 'lucide-react';
...
export function SafetyMenu({ feedbackId, userId, canDelete = false, onDeleted, onBlocked }: { feedbackId?: string; userId?: string; canDelete?: boolean; onDeleted?: () => void; onBlocked?: () => void }) {
  const { user } = useAuth();
    ...
    <div className="relative ml-auto">
      ...
      {open && (
        <div className="absolute bottom-full right-0 z-30 mb-1 w-44 overflow-hidden rounded-xl border border-border bg-surface-raised p-1 shadow-card-hover">
          ...
          {userId && userId !== user?.id && <button disabled={busy} onClick={block} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium text-danger hover:bg-danger/10"><ShieldBan className="h-3.5 w-3.5" /> Block user</button>}
          {canDelete && <button disabled={busy} onClick={removeReflection} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium text-danger hover:bg-danger/10"><Trash2 className="h-3.5 w-3.5" /> Delete reflection</button>}
        </div>
      )}
