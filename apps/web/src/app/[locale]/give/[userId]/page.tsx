...
export default function GiveFeedbackPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await feedbackApi.create({ receiverId: receiver.id, type, message, isPublic });
      setSubmitted(true);
  ...
  return (
    <div className="py-8 max-w-lg">
      ...
      {/* Receiver card */}
      <div className="flex items-center gap-3 mb-5 p-3.5 bg-gray-50 rounded-[14px] border border-gray-200">
        <Avatar displayName={receiver.displayName} username={receiver.username} avatarUrl={receiver.avatarUrl} size={48} />
        <div>
          <div className="font-bold text-[15px] text-gray-900">{receiver.displayName || receiver.username}</div>
      ...
      <div className="flex gap-2 mb-5">
        {FEEDBACK_TYPES.map((t) => (
          <button
            ...
            className="flex-1 py-2.5 rounded-[10px] text-[11px] font-semibold cursor-pointer transition-all"
            style={{
              border: `2px solid ${type === t.key ? t.color : '#E5E7EB'}`,
              background: type === t.key ? t.bg : '#fff',
              color: type === t.key ? t.textColor : '#6B7280',
            }}
          >
            {t.label}
          </button>
      ...
      {/* Public toggle */}
      <div className="flex items-center justify-between px-3.5 py-3 bg-gray-50 rounded-[10px] mb-5">
        <div>
          <div className="font-semibold text-[13px] text-gray-800">Make this public</div>
