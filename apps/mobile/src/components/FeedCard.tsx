...
export function FeedCard({ item, onGiverPress, onReceiverPress, onReport, onShare, canDelete, canEditPhoto, onDelete, onPhotoChanged }: Props) {
  const theme = useAppTheme(); const giver = item.giver; const receiver = item.receiver; const isJournalEntry = !item.receiverId;
  ...
  return <View style={[s.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
    ...
    <View style={s.body}><View style={s.header}>
      ...
      {(receiver || isJournalEntry) && <Text style={{ color: theme.muted }}>→</Text>}
      {receiver ? (
        <TouchableOpacity style={s.userRow} onPress={onReceiverPress} disabled={!onReceiverPress}><Avatar displayName={receiver.displayName} username={receiver.username} size={28} /><Text numberOfLines={1} style={[s.username, { color: theme.text }]}>{receiver.displayName || receiver.username}</Text></TouchableOpacity>
      ) : isJournalEntry ? (
        <Text numberOfLines={1} style={[s.username, { color: theme.muted, flexShrink: 1 }]}>{item.recipientName || 'someone'}</Text>
      ) : null}
    ...
    <View style={s.trailingGroup}>
      {isJournalEntry && <View style={[s.journalBadge, { backgroundColor: `${theme.accent}1A` }]}><Ionicons name="book-outline" size={11} color={theme.accent} /><Text style={[s.journalBadgeText, { color: theme.accent }]}>Journal{!item.isPublic && ' · private'}</Text></View>}
      {(onReport || canEditPhoto || canDelete) && <TouchableOpacity onPress={openOptions} style={s.reportButton} accessibilityLabel="More reflection options"><Ionicons name="ellipsis-horizontal" size={22} color={theme.muted} /></TouchableOpacity>}
    </View>
    </View>
